...
CODE_FILE_REGEX='(^|[/[:space:]])(skills/.+|scripts/.+|docs/.+|데스크탑용-skill파일/.+|\.claude/(settings\.json|harness\.config\.sh|hooks/.+\.sh|agents/.+\.md)|\.githooks/.+|\.github/workflows/.+\.ya?ml|\.gitattributes|CLAUDE\.md|AGENTS\.md|VERSION|EULA\.md|NOTICE\.md|README\.md|안내문\.md|시작-가이드-처음이라면-이것부터\.md|install\.(sh|py|ps1)|LICENSE\.md|시작-가이드\.md|\.claude-plugin/.+\.json)'
