...
#### 기존 거래처 — 패스트트랙 (질문 4개로 끝)

저장된 정보(사업자번호·상호·주소·이메일)는 **다시 묻지 않는다.** 홈택스 저장 거래처에서 불러오고, 화면에 채워진 상호가 사용자가 말한 곳과 맞는지 확인만 한다. 물을 것은 이 4개뿐:
...
- **월 정기·반복 거래면 더 단축:** "지난달과 동일 조건([품목] [금액])이면 '같아요'라고만 답하셔도 됩니다" → 다르면 달라진 항목만 받는다.
...
```text
처음 발행하는 거래처네요. 다음 6가지만 알려주시면 됩니다:
