...
5. **정합성 게이트 ②**: 입금완료 + 미입금 + 판단불가 = 청구 건수 — 불일치 시 보고 금지, 재점검. **입금완료는 사용자가 건별로 "맞다"고 답한 건만 센다.** 정확 일치·명의 일치 자동 매칭은 "입금 후보"이고 3분류에서는 **판단불가**다. 확인 응답을 받기 전 첫 보고의 입금완료는 항상 0이며, "입금완료 후보"라는 분류는 없다
