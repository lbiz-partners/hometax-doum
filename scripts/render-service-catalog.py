...
def main() -> int:
    ...
    rendered = render()
    if a.apply:
        target.write_text(rendered, encoding='utf-8'); print('✓ service-catalog.md 생성'); return 0
    if target.read_text(encoding='utf-8') != rendered:
        print('✗ service-catalog.md가 JSON 생성 결과와 다릅니다 — 행은 JSON, 산문은 scripts/service-catalog.template.md 를 고친 뒤 --apply', file=sys.stderr)
        return 1
